...
# ]
# tk = bmt.Toolkit()
# in_standard_color = "#FF5733"
# out_standard_color = "#33D4FF"
# intermediate_chebi_standard_color = "#33D4FF"
#
...
#
#
# def get_all_node_categories( kg_nodes, query_graph ):
#     qg_categories = {}
#     qgnodes = query_graph["nodes"]
#     for node, property in qgnodes.items():
#         if qgnodes[node].get("ids", []):
#             input_qc = qgnodes[node].get("categories", [])
#             if input_qc:
#                 qg_categories["input"] = input_qc[0]
#             else:
#                 node_ids = qgnodes[node]["ids"][0]
#                 qg_categories["input"] = kg_nodes[node_ids]["categories"][-1]
#         else:
#             output_qc = qgnodes[node].get("categories", [])
#             if output_qc:
#                 qg_categories["output"] = output_qc[0]
#
#     kgnode_categories = set()
#     for kg_node_prop in kg_nodes.values():
#         if qg_categories["input"] in (kg_node_prop["categories"]):
#             kgnode_categories.add(qg_categories["input"])
#         elif qg_categories["output"] in kg_node_prop["categories"]:
#             kgnode_categories.add(qg_categories["output"])
#         else:
#             kgnode_categories.add(kg_node_prop["categories"][-1])
#     node_categories = {"kg_categories": list(kgnode_categories), "qg_categories": qg_categories}
#     return node_categories
#
#
# def generate_color_map( categories, palette=color_palette ):
#     kg_categories = categories["kg_categories"]  # list of categories
#     qg_categories = categories["qg_categories"]  # dict of input and output kg categories
#     in_cat = qg_categories["input"]
#     out_cat = qg_categories["output"]
#     intermediate_chebi_cat = "biolink:ChemicalRole"
#
#
#     fixed_colors = {
#         in_cat: in_standard_color,
#         out_cat: out_standard_color,
#         intermediate_chebi_cat: out_standard_color,
#     }
#
#     # TODO
#     # check biolink for the sub and super categories of the input and output categories
#     # go back and store the qg_categories["input"] and ["output"] as a list of all the biolink hierarchies
#     # populate fixed colors with each of the items in each qg_categories values with input values assigned to in_standard_color and output values assigned to the out_standard_color
#     def get_subclasses( category ):
#         return tk.get_descendants(category, formatted=True, reflexive=False)
#
#     def get_superclass( category ):
#         return tk.get_ancestors(category, formatted=True, reflexive=False)
#
#     for cat in kg_categories:
#         if in_cat in get_superclass(cat):
#             fixed_colors[cat] = in_standard_color
#         if out_cat in get_superclass(cat):
#             fixed_colors[cat] = out_standard_color
#
#     dynamic_categories = [category for category in kg_categories if category not in fixed_colors]
#
...
#     """
#     Accepts json query graph
#     Returns its cytoscape graph
#     """
#     nodes = []
#     edges = []
...
#                           'style': {'background-color': in_standard_color},
...
#                           'style': {'background-color': out_standard_color},
...
#     """
#     Determines which of the support edge is enrich->group VS group->curie
#     """
...
#                             0])
#
#                 if edge["subject"] in terminals or edge["object"] in terminals:  # we are looking for the path  lookupresult--(biolink:member_of)-->uuid:1
#                     theedge = [kg_nodes[edge["object"]]["name"], 'has_member', kg_nodes[edge["subject"]]["name"]]
#                     subject = edge["subject"]
#                     object_ = edge["object"]
#                     if subject in terminals:
#                         next_element = terminals[terminals.index(subject) + 1] if subject in terminals and terminals.index(subject) + 1 < len(subject) else None
#                         finaledge = [kg_nodes[object_]["name"], group2curie_edge["predicate"], kg_nodes[next_element]["name"]]
...
#             source_color = get_node_color(category_colors, source_properties.get("categories", ["Unknown"])[-1])
#             target_color = get_node_color(category_colors, target_properties.get("categories", ["Unknown"])[-1])
#             source_shape = get_node_shape(node_categories, source_properties.get("categories", ["Unknown"])[-1])
#             target_shape = get_node_shape(node_categories, target_properties.get("categories", ["Unknown"])[-1])
#
...
#             elements.append({'data': sourcedata, 'position': {'x': position_x, 'y': position_y},
#                              'style': {'width': node_size, 'height': node_size, 'background-color': source_color,
#                                        'shape': source_shape}})
#             targetdata = {'id': target, 'label': f"{target_properties['name']} ({target})"}
#             targetdata.update(target_properties)
#             elements.append({'data': targetdata, 'position': {'x': position_x + 200, 'y': position_y},
#                              'style': {'background-color': target_color, 'shape': target_shape}})
#
...
#     """
#     Generates rule graphs based on selected inference edges.
#         selected_inference_edge: the edeg between the inferred results and input curie
#         kg_nodes: Knowledge graph nodes
#         kg_edges: knowledge graph edges
#         aux_graphs: Auxiliary graphs
#
#     returns:
#         enriched2grouplist: the enrichment nodes mapped to the enriched group
#         lookup_lists: the members of the enriched group
#         pvalues: PValue significance for each enrichment
#
#     """
#     lookup_lists = []
#     enriched2grouplist = []
...
#     support_graphs = [attributes["value"] for attributes in kginference_edge["attributes"] if attributes["attribute_type_id"] == "biolink:support_graphs"]
...
#         ac_dict = json.loads(answerset_json)
#         qg = ac_dict["message"]["query_graph"]
#     dcc.Store(id='answerset-input', data=answerset_json),
#     layout = dbc.Container([html.Div([
#         dcc.Store(id='stored-node-categories'), dcc.Store(id='stored-category-colors'), dcc.Store(id='stored-kg-nodes'),
#         dcc.Store(id='stored-kg-edges'), dcc.Store(id='stored-aux-graphs'), dcc.Store(id='stored-qg'),
#         dcc.Store(id='stored-results'), dcc.Store(id='stored-inferred-df'),
#         dbc.Row([
#             dbc.Card(
...
#             dbc.Col(html.Div(
#                 [html.P("select a row(s) to see the >>inference path", style={'background-color': '#cbd3dd'}),
#                  html.Div(id='result-table-container', children=[
...
#                  ])]), width=10)]),
#         dbc.Row(
#             [dbc.Col(html.Div(id='cytoscape-cards'), width=8), dbc.Col(html.Div(id='edge-data-table-div'), width=4)]),
...
#     node_categories = get_all_node_categories(kg_nodes, query_graph)
#     category_colors = generate_color_map(node_categories)
#     # print('category_colors', category_colors)
#     kg_categories = node_categories["kg_categories"]
#     df = get_inferred_result_df(kg_edges, kg_nodes, results)
#     return query_graph, kg_nodes, kg_edges, results, aux_graphs, kg_categories, category_colors, df.to_json( orient='split')
#
#
...
#         elements_list, enriched2grouplist, lookup_lists = generate_elements(result, kg_nodes, kg_edges, aux_graphs,
#                                                                             node_categories, category_colors)
...
#                         layout={'name': 'breadthfirst', 'idealEdgeLength': 3, 'nodeRepulsion': 10,
#                                 'edgeElasticity': 0.45, 'nestingFactor': 0, 'gravity': 1, 'numIter': 1000},
#                         stylesheet=[{'selector': 'node', 'style': {'label': 'data(label)'}},
#                                     {'selector': 'edge',
...
#                         html.Div(html.P("select an node or edge to view its support graph",
...
#
# ## Tap Node #####
# @callback(Output('edge-data-table-div', 'children', allow_duplicate=True), Input({'type': 'cytoscape', 'index': ALL}, 'tapNode'), prevent_initial_call=True)
# def store_edge_data( node_data ):
#     if node_data and any(node_data):
#         return node_data[0]['data']
#     return html.Div()
#
#
# ## Tap Edge #####
...
#         return html.Div("This edge has no support graphs")
#
...
#         lookup_table_output = html.Div(
#             [html.P("LOOKUP Members ↓ ", style={'backgroundColor': '#cbd3dd'}), lookup_table_component])
...
#         enrichment = pd.DataFrame(enrichment_baskets,
#                                   columns=["Subject", "Predicate1", "Object", "Pvalue", "Knowledge_Source"])
...
#         enrich_table_output = html.Div(
#             [html.P(f"RULE(s) ↓ for the {len(enrichment)} paths", style={'backgroundColor': '#cbd3dd'}),
#              enrich_table_component])
...
def vizlayout(answerset):
    ...
    if isinstance(answerset, str):
        qg = json.loads(answerset)["message"]["query_graph"]
        answerset_json = answerset
    layout = dbc.Container([html.Div([
        dcc.Store(id='answerset-input', data=answerset_json),
...
def inferrence(df_json):
    ...
    return dash_table.DataTable(id='result-table',
                        columns=[{"name": i, "id": i} for i in dbf.columns],
                        data=dbf.to_dict('records'),
                    )
